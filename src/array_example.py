...
def input_args():
    ...
    parser.add_argument( "--array-size", "-s", dest="array_size", type=int, default=num_physical_cores )
    args = parser.parse_args()
    return args

## Base Model
def create_base_model( array_size ):
    cubit.cmd( "reset" )
    cubit.cmd( "bri x 1" )
    cubit.cmd( f"vol all copy move x 2 repeat {array_size - 1}" )
    cubit.cmd( f"vol all copy move y 2 repeat {array_size - 1}" )
    cubit.cmd( f"vol all copy move z 2 repeat {array_size - 1}" )
